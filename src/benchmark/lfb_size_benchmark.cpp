...
struct LFBBenchmarkConfig
{
    ...
    size_t num_prefetch_instructions;
...
int main(int argc, char **argv)
{
    ...
    benchmark_config.add_options()
        ...
        ("num_prefetch_instructions", "Number of distinct prefetch instructions issued per measurement", cxxopts::value<std::vector<size_t>>()->default_value("10"))
    ...
    for (auto &runtime_config : benchmark_config.get_runtime_configs())
    {
        ...
        auto num_prefetch_instructions = convert<size_t>(runtime_config["num_prefetch_instructions"]);
        ...
        results["config"]["num_prefetch_instructions"] = config.num_prefetch_instructions;
