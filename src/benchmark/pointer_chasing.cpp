...
void pointer_chase(size_t i, auto &config, auto &data, auto &durations)
{
    ...
    {
        curr_pointers[i] = uniform_dis(gen);
    }
    ...
    for (size_t r = 0; r < config.num_resolves; ++r)
    {
        ...
        for (size_t i = 0; i < config.num_parallel_pc; ++i)
        {
        ...
        for (size_t i = 0; i < config.num_parallel_pc; ++i)
        {
            curr_pointers[i] = *(data.data() + curr_pointers[i]);
        }
        duration += end - start;
        // LFB CLEAR
        // idle computation
